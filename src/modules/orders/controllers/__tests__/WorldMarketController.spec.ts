...
describe("handle", () => {
  it("should call usecase with correct params", async () => {
    ...
    jest
      .spyOn(controller, "worldMarketPaymentMethodToUtmifyPaymentMethod")
      .mockReturnValueOnce(mockedMethod);

    const mockedStatus = UtmifyTransactionStatus.Paid;
    jest
      .spyOn(controller, "worldMarketStatusToUtmifyTransactionStatus")
      .mockReturnValueOnce(mockedStatus);

    const mockedProducts: UtmifyProduct[] = [];
    jest
      .spyOn(controller, "worldMarketProductsToUtmifyProducts")
    ...
    jest
      .spyOn(controller, "worldMarketCustomerToUtmifyCustomer")
    ...
    jest
      .spyOn(controller, "worldMarketBodyToUtmifyValues")
    ...
    expect(
      controller.worldMarketPaymentMethodToUtmifyPaymentMethod
    ).toHaveBeenCalledTimes(1);
    expect(
      controller.worldMarketStatusToUtmifyTransactionStatus
    ).toHaveBeenCalledTimes(1);
    expect(
      controller.worldMarketProductsToUtmifyProducts
    ).toHaveBeenCalledTimes(1);
    expect(
      controller.worldMarketCustomerToUtmifyCustomer
    ).toHaveBeenCalledTimes(1);
    ...
    expect(
      controller.worldMarketStatusToUtmifyTransactionStatus
    ).toHaveBeenCalledWith(body.order_status);
    expect(controller.worldMarketProductsToUtmifyProducts).toHaveBeenCalledWith(
      body.order_details.products
    );
    expect(controller.worldMarketCustomerToUtmifyCustomer).toHaveBeenCalledWith(
      body.customer
    );
    expect(controller.worldMarketBodyToUtmifyValues).toHaveBeenCalledWith(
      body.order_details
    );
  });
});
...
describe("worldMarketPaymentMethodToUtmifyPaymentMethod", () => {
  it("should return correct utmify payment method", () => {
    expect(
      controller.worldMarketPaymentMethodToUtmifyPaymentMethod("pix")
    ).toBe(UtmifyPaymentMethod.Pix);

    expect(
      controller.worldMarketPaymentMethodToUtmifyPaymentMethod("boleto")
    ).toBe(UtmifyPaymentMethod.Billet);

    expect(
      controller.worldMarketPaymentMethodToUtmifyPaymentMethod("credit_card")
...
describe("worldMarketStatusToUtmifyTransactionStatus", () => {
  it("should return correct utmify transaction status", () => {
    expect(
      controller.worldMarketStatusToUtmifyTransactionStatus("pending")
    ).toBe(UtmifyTransactionStatus.Pending);

    expect(
      controller.worldMarketStatusToUtmifyTransactionStatus("approved")
    ).toBe(UtmifyTransactionStatus.Paid);

    expect(
      controller.worldMarketStatusToUtmifyTransactionStatus("refunded")
...
describe("worldMarketProductsToUtmifyProducts", () => {
  it("should return correct utmify products", () => {
    ...
    expect(
      controller.worldMarketProductsToUtmifyProducts(worldMarketProducts)
    ).toEqual([
      {
        id: "12k3hkahsd",
        name: "T-Shirt",
...
describe("worldMarketCustomerToUtmifyCustomer", () => {
  it("should return correct utmify customer", () => {
    ...
    expect(
      controller.worldMarketCustomerToUtmifyCustomer(worldMarketCustomer)
    ).toEqual({
