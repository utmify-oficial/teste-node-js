...
export class UtmifyOrdersRepositoryMongoose implements UtmifyOrdersRepository {
  ...
  async findBySaleId(order: UtmifyOrder): Promise<UtmifyOrderFromDb | null> {
    const model = new UtmifyOrderModel(order);
    const foundOrder = await UtmifyOrderModel.findOne({ saleId: order.saleId });

    return foundOrder?.toEntity() ?? null;
  }
}
